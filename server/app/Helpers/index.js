 ...
 * então 'public/properties' será utilizado.
...
const manage_single_upload = async (file, path = null) => {
  path = path ? path : Helpers.publicPath('storage')
  // gera um nome aleatório
  const random_name = await str_random(5)
 ...
 * @return { Object }
 */
const manage_multiple_uploads = async (files, path = null, groupName = '') => {
  if (groupName) groupName = `/${groupName.replace(/\%20/g, ' ')}`;
  path = path ? path : Helpers.appRoot(`storage${groupName}`);
  let successes = [],
    errors = []

  await Promise.all(
    files.map(async file => {
      let random_name = await str_random(5)
      let filename = `${new Date().getTime()}-${random_name}.${file.subtype}`
      // move o arquivo
      await file.move(path, {
        name: filename
